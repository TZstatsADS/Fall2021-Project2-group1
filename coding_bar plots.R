...
ggplot(data=df_open) +
  ...
  labs(title = "Open Violation",x="Date",y="Violation Number",fill="Violation Status")+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(hjust = 0.5, angle = 45))

ggplot(data=df_close) +
  geom_bar(aes(x=factor(date)),col="blue",fill="white")+
  labs(title = "Closed Violation",x="Date",y="Violation Number",fill="Violation Status")+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(hjust = 0.5, angle = 45))

ggplot(data=df) +
  geom_bar(aes(x=factor(date),fill=factor(ViolationStatus)))+
  labs(title = "Violation Status",x="Date",y="Violation Number",fill="Violation Status")+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(hjust = 0.5, angle = 45))

ggplot(data=df) +
  geom_bar(aes(x=factor(Borough),fill=factor(ViolationStatus)))+
  labs(title = "Violation Status",x="Borough",y="Violation Number",fill="Violation Status")+
  theme(plot.title = element_text(hjust = 0.5),axis.text.x = element_text(hjust = 0.5, angle = 45))
